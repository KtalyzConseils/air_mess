...
const PAYMENT_COPY = {
  fr: {
    missingPaymentId: 'Identifiant de paiement manquant.',
    stillConfirming: 'Le paiement est encore en cours de confirmation.',
    confirmationFailed: 'Confirmation impossible.',
    invalidLink: 'Lien de paiement invalide.',
    back: 'Revenir',
    close: 'Fermer le paiement',
    title: 'Paiement securise',
    retry: 'Verifier a nouveau',
  },
  en: {
    missingPaymentId: 'Missing payment identifier.',
    stillConfirming: 'The payment is still being confirmed.',
    confirmationFailed: 'Unable to confirm.',
...
export default function PaymentScreen() {
  ...
  const copy = PAYMENT_COPY[language]
  ...
  const finishPayment = async () => {
    ...
    try {
      ...
      if (!id) throw new Error(copy.missingPaymentId)

      const confirmation = await waitForPaymentConfirmation(id)
      if (confirmation.status !== 'paid') {
        throw new Error(copy.stillConfirming)
      }
      ...
      setLoading(false)
      setConfirmationError(error instanceof Error ? error.message : copy.confirmationFailed)
    }
  }
  ...
  if (!checkoutUrl) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-cream px-6">
        <Text className="text-center text-base font-bold text-airmess-red">{copy.invalidLink}</Text>
        <Pressable onPress={() => router.back()} className="mt-5 rounded-2xl bg-airmess-yellow px-5 py-4">
          <Text className="font-extrabold text-ink">{copy.back}</Text>
        </Pressable>
      </SafeAreaView>
    )
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-cream">
      <View className="h-14 flex-row items-center border-b border-warm-200 px-4">
        <Pressable onPress={() => router.back()} className="h-10 w-10 items-center justify-center" accessibilityLabel={copy.close}>
          <Ionicons name="close" size={25} color="#1A1614" />
        </Pressable>
        <Text className="flex-1 text-center text-base font-extrabold text-ink">{copy.title}</Text>
      ...
      {confirmationError ? (
        <View className="absolute bottom-0 left-0 right-0 border-t border-warm-200 bg-cream p-5">
          ...
          <Pressable onPress={() => void finishPayment()} className="mt-3 items-center rounded-2xl bg-airmess-yellow px-5 py-4">
            <Text className="font-extrabold text-ink">{copy.retry}</Text>
          </Pressable>
        </View>
      ) : null}
    </SafeAreaView>
