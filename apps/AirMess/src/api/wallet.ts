...
export async function confirmPayment(paymentId: number): Promise<PaymentConfirmation> {
  const { data } = await api.post<PaymentConfirmation>(`/me/payments/${paymentId}/confirm`)
  return data
}
