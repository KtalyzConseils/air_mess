...
export default function PushDiagnosticsScreen() {
  ...
  async function resync() {
    ...
    await refresh()
    Alert.alert(
      ok ? 'Synchronise' : 'Echec',
      ok
        ? 'Ce telephone est enregistre pour les notifications.'
        : 'Verifie Internet et les autorisations, puis reessaie.',
    )
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-cream">
      <View className="flex-row items-center px-5 py-3">
        ...
        <Text className="text-xl font-jk-extrabold text-ink ml-2">
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }}>
        ...
        {result ? (
          <View className="bg-off-white border border-warm-200 rounded-2xl overflow-hidden">
            ...
            <Status label="Token sur le telephone" ok={result.tokenStored} />
            <Status label="Token enregistre sur le serveur" ok={result.tokenRegistered} />
            <Status label="Canal appels de course" ok={result.incomingChannelEnabled === true} />
            <Status
              label="Derniere synchronisation"
              ok={Boolean(result.lastSync)}
              value={result.lastSync ? new Date(result.lastSync).toLocaleString('fr-FR') : 'Jamais'}
              last
            />
          </View>
        ) : null}

        <Action title="Resynchroniser ce telephone" onPress={() => { void resync() }} />
        <Action title="Tester une notification locale" onPress={() => { void showLocalPushTest() }} />
        <Action title="Reglages plein ecran" onPress={() => { void openFullScreenIntentSettings() }} />
      </ScrollView>
    </SafeAreaView>
  )
}
