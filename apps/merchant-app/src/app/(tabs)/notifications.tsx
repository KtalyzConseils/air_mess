...
export default function NotificationsScreen() {
  ...
  // L'utilisateur consulte ses notifs : la pastille de l'icône de l'app n'a plus lieu d'être.
  useEffect(() => {
    clearBadge()
  }, [])
...
function NotificationRow({ item }: { item: NotificationItem }) {
  ...
  // Tap : on marque lu et, si la notif porte sur une course, on l'ouvre.
  function handlePress() {
    if (!isRead) mut.mutate()
    if (item.course_id) router.push(`/course/${item.course_id}` as never)
  }
